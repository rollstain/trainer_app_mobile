package app.trainer.feature.progress.presentation.photos.ui

import androidx.compose.runtime.Composable
import androidx.compose.runtime.getValue
import app.trainer.base.failure.toastMessage
import app.trainer.feature.progress.presentation.photos.mvi.PhotoCompareScreenModel
import app.trainer.feature.progress.presentation.photos.mvi.PhotoCompareSideEffect
import app.trainer.feature.progress.presentation.photos.mvi.PhotoOwner
import app.trainer.navigation.LocalNavigator
import app.trainer.navigation.Navigator
import app.trainer.navigation.Screen
import app.trainer.navigation.currentOrThrow
import app.trainer.navigation.koinScreenModel
import app.trainer.uikit.widgets.LocalToastHost
import app.trainer.uikit.widgets.ToastHostState
import org.koin.core.parameter.parametersOf

class PhotoCompareScreen(private val owner: PhotoOwner) : Screen {

    @Composable
    override fun Content() {
        val navigator: Navigator = LocalNavigator.currentOrThrow
        val toastHost: ToastHostState = LocalToastHost.current
        val screenModel: PhotoCompareScreenModel = koinScreenModel(
            parameters = { parametersOf(owner) },
        )
        val state by screenModel.collectAsState()

        PhotoCompareView(
            state = state,
            onEvent = { screenModel.dispatch(event = it) },
            onBackClick = navigator::pop,
        )

        screenModel.collectSideEffect { effect ->
            when (effect) {
                is PhotoCompareSideEffect.ShowFailure -> toastHost.show(effect.failure.toastMessage())
            }
        }
    }
}
